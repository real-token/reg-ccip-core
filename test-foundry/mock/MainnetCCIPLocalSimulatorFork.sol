...
import {MainnetRegister} from "./MainnetRegister.sol";
...
contract MainnetCCIPLocalSimulatorFork is Test {
    /**
    ...
    MainnetRegister immutable i_register;
    ...
    constructor() {
        vm.recordLogs();
        i_register = new MainnetRegister();
    ...
    function switchChainAndRouteMessage(uint256 forkId) external {
        ...
        address sourceRouterAddress = i_register
    ...
    function getNetworkDetails(
        uint256 chainId
    ) external view returns (MainnetRegister.NetworkDetails memory) {
    ...
    function setNetworkDetails(
        uint256 chainId,
        MainnetRegister.NetworkDetails memory networkDetails
    ) external {
    ...
    function _routeCapturedMessages(
        ...
        for (uint256 i; i < logsLength; ++i) {
            ...
            if (entries[i].topics[0] == CCIPSendRequested.selector) {
                InternalPreV1dot6.EVM2EVMMessage memory message = abi.decode(
                    entries[i].data,
                    (InternalPreV1dot6.EVM2EVMMessage)
                );

                if (!s_processedMessages[message.messageId]) {
                    for (uint256 j; j < forkIds.length; ++j) {
                        vm.selectFork(forkIds[j]);
                        uint64 destinationChainSelector = i_register
                            .getNetworkDetails(block.chainid)
                            .chainSelector;

                        vm.selectFork(sourceForkId);
                        address onRampContract = IRouterFork(
                            sourceRouterAddress
                        ...
                        if (entries[i].emitter == onRampContract) {
                            ...
                            for (uint256 k = offRampsLength; k > 0; --k) {
                                if (
                                    offRamps[k - 1].sourceChainSelector ==
                                    message.sourceChainSelector
                                ) {
                                    vm.startPrank(offRamps[k - 1].offRamp);
                                    uint256 numberOfTokens = message
                                        .tokenAmounts
                                        .length;
                                    bytes[]
                                        memory offchainTokenData = new bytes[](
                                            numberOfTokens
                                        );
                                    uint32[]
                                        memory tokenGasOverrides = new uint32[](
                                            numberOfTokens
                                        );
                                    for (uint256 l; l < numberOfTokens; ++l) {
                                        tokenGasOverrides[l] = uint32(
                                            message.gasLimit
                                        );
                                    }
                                    try
                                        IEVM2EVMOffRampPreV1dot6Fork(
                                            offRamps[k - 1].offRamp
            ...
            if (entries[i].topics[0] == CCIPMessageSent.selector) {
                Internal.EVM2AnyRampMessage memory message = abi.decode(
                    entries[i].data,
                        ...
                        if (entries[i].emitter == onRampContract) {
                            ...
                            for (uint256 k = offRampsLength; k > 0; --k) {
                                if (
                                    ...
                                    vm.startPrank(offRamps[k - 1].offRamp);
                                    uint256 gasLimit = _fromBytes(
                                        message.extraArgs
                                    ).gasLimit;
                                    uint256 numberOfTokens = message
                                        .tokenAmounts
                                        .length;
                                    Internal.Any2EVMTokenTransfer[]
                                        memory tokenAmounts = new Internal.Any2EVMTokenTransfer[](
                                            numberOfTokens
                                        );
                                    for (uint256 l; l < numberOfTokens; ++l) {
                                        tokenAmounts[l] = Internal
                                            .Any2EVMTokenTransfer({
                                                sourcePoolAddress: abi
                                                    .encodePacked(
                                                        message
                                                            .tokenAmounts[l]
                                                            .sourcePoolAddress
                                    ...
                                    Internal.Any2EVMRampMessage
                                        memory any2EVMRampMessage = Internal
                                            .Any2EVMRampMessage({
    ...
    function _fromBytes(
        ...
        if (extraArgsTag == Client.GENERIC_EXTRA_ARGS_V2_TAG) {
            ...
            return
                Client.GenericExtraArgsV2({
                    gasLimit: abi.decode(gasLimit, (uint256)),
                    allowOutOfOrderExecution: false
                });
        }
